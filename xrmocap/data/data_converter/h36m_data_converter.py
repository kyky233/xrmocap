...
from xrmocap.utils.data_io import load_pickle_file
...
class H36mDataConverter(BaseDataCovnerter):
    """
    convert human36d from original format to unified format
    the Original data anno has already been split into train and validation set separately. (split by subject)
    the whole anno file can be seen as a list of dict.
    each dict is the annotation of one image,
    where dict_keys(['image', 'joints_2d', 'joints_3d', 'joints_3d_camera', 'joints_vis', 'video_id', 'image_id', \
    ...
    def __init__(self,
                 ...
                 subject_idxs: Union[str, List[int]] = 'all',
        ...
        Args:
            data_root (str):
                Path to the Human36m dataset.
            dataset_type (str):
                define which annotation file to load
                'train' or 'validation'
            bbox_detector (Union[dict, None]):    - don't need it
                A human bbox_detector, or its config, or None.
                If None, converting perception 2d will be skipped.
                Defaults to None.
            kps2d_estimator (Union[dict, None]):    - don't need it
            ...
            subject_idxs (List[str], optional):
                A list of subject directory idxs, from 1 to 11
                [1, 5, 6, 7, 8, 9, 11] in train.pkl, others in validation.pkl
                Defaults to 'all', all subjects in train/validation pickle file will be selected.
            view_idxs (List[int], optional):    # can be either str or int
                A list of selected view indexes in each scene, for h36m, only 4 cameras can be obtained(0~3)
                like [[0, 1], [0, 2, 3]].
        ...
        origin_anno_path = os.path.join('annot', 'h36m_'+dataset_type+'.pkl')
        self.anno_origin = load_pickle_file(origin_anno_path)   # load original annotation pickle file
        ...
        if subject_idxs == 'all':
            self.subject_idxs = self.list_subject_idxs()
        else:
            self.subject_idxs = subject_idxs
        if scene_range == 'all':
            self.scene_range = None
        else:
    ...
    def list_subject_idxs(self) -> List[int]:
        """Get the list of subject indexes in original annotation pickle file.

        Returns:
            List[int]: The list of subject indexes.
        """
        subject_idxs = []
        for single_item in self.anno_origin:
            subject_idx = single_item['subject']
            if subject_idx not in subject_idxs:
